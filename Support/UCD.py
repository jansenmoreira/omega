...
parser = argparse.ArgumentParser(description="Generate UCD.c")
parser.add_argument("--ucd", required=True, help="Zip file that contains 'ucd.all.flat.xml'")
parser.add_argument("--output", required=True, help="Folder to output UCD.c")
...
with open(os.path.join(args.output, "UCD.c"), "w") as file:
    file.write("#include <Support/Unicode.h>\n\n")
